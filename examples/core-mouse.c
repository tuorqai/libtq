...
static int current_bubble = 0;     // Index that should be used next.
static int active_bubble = -1;     // Index of active bubble (-1: none)
static float crosshair_size = 8;   // Size of crosshair, in pixels
...
static void on_button_down(tq_vec2i pos, tq_mouse_button button)
{
...
static void on_button_up(tq_vec2i pos, tq_mouse_button button)
{
    if (button == TQ_MOUSE_BUTTON_LEFT) {
        active_bubble = -1;
    }
...
// TODO: comments.
static void on_wheel_scroll(tq_vec2i cursor, tq_vec2f wheel)
{
